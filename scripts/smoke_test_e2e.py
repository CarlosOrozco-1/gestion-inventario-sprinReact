...
def main():
    ...
    jefe = request("POST", "/usuarios/admin", admin_tok, {"nombre": "Jefa E2E", "email": f"jefe-e2e-{SUF}@inventario.com", "password": "123456", "rol": "JEFE"}, 200)
    aux = request("POST", "/usuarios/admin", admin_tok, {"nombre": "Aux E2E", "email": f"aux-e2e-{SUF}@inventario.com", "password": "123456", "rol": "AUXILIAR"}, 200)
    jefe_id, aux_id = jefe["id"], aux["id"]
    check("se crearon jefe y auxiliar", jefe["rol"]["nombre"] == "JEFE" and aux["rol"]["nombre"] == "AUXILIAR")
    ...
    # Update nombre/email (corrección de nombre tipo "Maria" -> "Marhia")
    request("PUT", f"/usuarios/admin/{jefe_id}", admin_tok, {"nombre": "Marhia E2E", "email": f"jefe-e2e-{SUF}@inventario.com"})
    jefe_act = request("GET", "/usuarios/admin", admin_tok)
    check("el nombre editado se refleja en el listado", any(u["id"] == jefe_id and u["nombre"] == "Marhia E2E" for u in jefe_act))
    # Email duplicado -> 400
    request("PUT", f"/usuarios/admin/{aux_id}", admin_tok, {"nombre": "Aux E2E", "email": "admin@inventario.com"}, 400)
    # Cambio de rol
    request("PUT", f"/usuarios/admin/{jefe_id}/rol", admin_tok, {"rol": "AUXILIAR"})
    jefe_act = request("GET", "/usuarios/admin", admin_tok)
    check("cambio de rol JEFE->AUXILIAR aplicado", any(u["id"] == jefe_id and u["rol"]["nombre"] == "AUXILIAR" for u in jefe_act))
    request("PUT", f"/usuarios/admin/{jefe_id}/rol", admin_tok, {"rol": "JEFE"})
    # Suspend/activate
    request("PUT", f"/usuarios/admin/{aux_id}/status", admin_tok, {"activo": False})
    jefe_act = request("GET", "/usuarios/admin", admin_tok)
    check("suspender usuario activo=false", any(u["id"] == aux_id and not u["activo"] for u in jefe_act))
    request("PUT", f"/usuarios/admin/{aux_id}/status", admin_tok, {"activo": True})
    ...
    check("listado de movimientos incluye los E2E", any(m["insumoNombre"] == f"Gasa E2E {SUF}" for m in movs))
    ...
    proys = [{"insumo": "Gasa E2E", "stock": 10, "stockMinimo": 20, "stockMaximo": 60, "costoEstimado": 30.0, "deficit": 50, "inversionNecesaria": 1500.0, "urgencia": "Alta"},
             {"insumo": "Jeringa E2E", "stock": 5, "stockMinimo": 5, "stockMaximo": 100, "costoEstimado": 10.0, "deficit": 95, "inversionNecesaria": 950.0, "urgencia": "Alta"}]
