...
export default function AlertasModule({ insumos }) {
    ...
    <div className="module-fade-in">
      ...
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '1.6rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span>🚨</span> Módulo de Alertas de Inventario
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', marginTop: '2px' }}>
          Monitoreo en tiempo real de existencia de insumos, stock bajo y desabastecimiento
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '32px' }}>
        <div className="glass-card sharp-border" style={{ padding: '20px' }}>
          <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Total de Productos</div>
          <div style={{ fontSize: '2.2rem', fontWeight: 700, color: 'var(--accent-blue)', marginTop: '4px' }}>{totalInsumos}</div>
        </div>

        <div className="glass-card sharp-border" style={{ padding: '20px' }}>
          <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Unidades Totales en Stock</div>
          <div style={{ fontSize: '2.2rem', fontWeight: 700, color: 'var(--accent-cyan)', marginTop: '4px' }}>{totalStock}</div>
        </div>

        <div className="glass-card sharp-border" style={{ padding: '20px' }}>
          <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Insumos con Stock Bajo</div>
          <div style={{ fontSize: '2.2rem', fontWeight: 700, color: 'var(--accent-amber)', marginTop: '4px' }}>{insumosBajos.length}</div>
        </div>

        <div className="glass-card sharp-border" style={{ padding: '20px' }}>
          <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Insumos Agotados</div>
